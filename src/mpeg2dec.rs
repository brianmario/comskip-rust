...
#![feature(asm, extern_types, label_break_value, register_tool)]
use std::arch::asm;
extern "C" {
    pub type __sFILEX;
    ...
    fn av_frame_set_best_effort_timestamp(frame: *mut AVFrame, val: int64_t);
    ...
    fn av_codec_is_decoder(codec: *const AVCodec) -> libc::c_int;
    fn av_codec_get_max_lowres(codec: *const AVCodec) -> libc::c_int;
    fn av_codec_next(c: *const AVCodec) -> *mut AVCodec;
    ...
    static mut use_cuvid: libc::c_int;
    static mut use_vdpau: libc::c_int;
    static mut use_dxva2: libc::c_int;
    ...
    fn set_fps(frame_delay: libc::c_double);
...
#[no_mangle]
pub static mut myoptions: *mut AVDictionary = 0 as *const AVDictionary
    as *mut AVDictionary;
#[no_mangle]
pub static mut global_video_state: *mut VideoState = 0 as *const VideoState
    as *mut VideoState;
#[no_mangle]
...
static mut audio_buffer: [libc::c_short; 1600000] = [0; 1600000];
static mut audio_buffer_ptr: *mut libc::c_short = unsafe {
    audio_buffer.as_ptr() as *mut _
};
...
#[no_mangle]
pub unsafe extern "C" fn list_codecs() {
    let mut p: *mut AVCodec = 0 as *mut AVCodec;
    let mut i: libc::c_int = 0 as libc::c_int;
    avcodec_register_all();
    p = av_codec_next(0 as *const AVCodec);
    printf(b"Decoders:\n\0" as *const u8 as *const libc::c_char);
    printf(b"---------\n\0" as *const u8 as *const libc::c_char);
    while !p.is_null() {
        if av_codec_is_decoder(p) != 0 {
            printf(b"%s\0" as *const u8 as *const libc::c_char, (*p).name);
            i = (i as libc::c_ulong).wrapping_add(strlen((*p).name)) as libc::c_int
                as libc::c_int;
            if i > 80 as libc::c_int {
                printf(b"\n\0" as *const u8 as *const libc::c_char);
                i = 0 as libc::c_int;
            } else {
...
pub unsafe extern "C" fn retreive_frame_volume(
    ...
    if s_per_frame > 1 as libc::c_int && base_apts <= from_pts && to_pts < top_apts {
        ...
        if !timing_file.is_null() && csStepping == 0 && csJumping == 0
            && csStartJump == 0
        {
            fprintf(
                timing_file,
...
pub unsafe extern "C" fn backfill_frame_volumes() {
    ...
    let mut local_initial_pts: libc::c_double = initial_pts;
    if framenum < 3 as libc::c_int {
        return;
    }
    f = framenum - 2 as libc::c_int;
    if abs(local_initial_pts as libc::c_int) > 200 as libc::c_int {
        local_initial_pts = 0 as libc::c_int as libc::c_double;
    }
    while get_frame_pts(f) + local_initial_pts > base_apts && f > 1 as libc::c_int {
        f -= 1;
    }
    while f < framenum - 1 as libc::c_int
        && get_frame_pts(f + 1 as libc::c_int) + local_initial_pts <= top_apts
        && top_apts - base_apts > 0.2f64
    {
        volume = retreive_frame_volume(
            fmax(get_frame_pts(f) + local_initial_pts, base_apts),
            get_frame_pts(f + 1 as libc::c_int) + local_initial_pts,
        );
...
pub unsafe extern "C" fn sound_to_frames(
    ...
    audio_samples = audio_buffer_ptr.offset_from(audio_buffer.as_mut_ptr())
    ...
    if old_sample_rate == (*(*(*is_0).audio_st).codec).sample_rate
        && ((audio_buffer_ptr.offset_from(audio_buffer.as_mut_ptr()) as libc::c_long)
            ...
            || audio_buffer_ptr.offset_from(audio_buffer.as_mut_ptr()) as libc::c_long
                >= 1600000 as libc::c_int as libc::c_long
            || (top_apts - base_apts)
                * ((*(*(*is_0).audio_st).codec).sample_rate as libc::c_double + 0.5f64)
                > 1600000 as libc::c_int as libc::c_double || top_apts < base_apts
            || !(fabs(
                audio_samples as libc::c_double
                    / ((*(*(*is_0).audio_st).codec).sample_rate as libc::c_double
                        + 0.5f64) + base_apts - top_apts,
            ) < 0.001f64) || audio_samples < 0 as libc::c_int
            || audio_samples >= 1600000 as libc::c_int)
    {
        Debug(
            1 as libc::c_int,
    ...
    if s + audio_samples > 1600000 as libc::c_int {
        Debug(
            1 as libc::c_int,
            ...
            while i < s {
                ...
                if planar != 0 {
                    ...
                    while l < (*(*(*is_0).audio_st).codec).channels {
                        ...
                        fb[0 as libc::c_int
                            as usize] = (fb[0 as libc::c_int as usize]).offset(1);
                        volume = (volume as libc::c_float
                ...
                audio_buffer_ptr = audio_buffer_ptr.offset(1);
                *fresh4 = (volume / (*(*(*is_0).audio_st).codec).channels)
                    as libc::c_short;
                avg_volume
                    += abs(volume / (*(*(*is_0).audio_st).codec).channels)
                        as libc::c_double;
                i += 1;
            }
        } else {
            ...
            while i < s {
                ...
                if planar != 0 {
                    ...
                    while l < (*(*(*is_0).audio_st).codec).channels {
                        ...
                        sb[0 as libc::c_int
                            as usize] = (sb[0 as libc::c_int as usize]).offset(1);
                ...
                audio_buffer_ptr = audio_buffer_ptr.offset(1);
                *fresh7 = (volume / (*(*(*is_0).audio_st).codec).channels)
                    as libc::c_short;
                avg_volume
                    += abs(volume / (*(*(*is_0).audio_st).codec).channels)
                        as libc::c_double;
...
#[no_mangle]
pub unsafe extern "C" fn audio_packet_process(
    mut is_0: *mut VideoState,
    ...
    *fresh8 = (*pkt).data;
            ...
            == AV_CODEC_ID_AC3 as libc::c_int as libc::c_uint
    {
        ...
        memcpy(
            &mut *ac3_packet.as_mut_ptr().offset(ac3_packet_index as isize)
        ...
        *fresh9 = ac3_packet.as_mut_ptr();
        ...
        if rps >= 2 as libc::c_int {
            ...
            rps = (*pkt_temp).size;
            pp = &mut *((*pkt_temp).data).offset(0 as libc::c_int as isize)
                as *mut uint8_t;
    ...
    if (*pkt).pts != 0x8000000000000000 as libc::c_ulonglong as int64_t {
        ...
        (*is_0)
            .audio_clock = av_q2d((*(*is_0).audio_st).time_base)
            * ((*pkt).pts
        ...
        if initial_apts_set != 0 && (*is_0).audio_clock != 0.0f64
            && fabs((*is_0).audio_clock - prev_audio_clock) > 0.02f64
        {
            if do_audio_repair != 0
                && fabs((*is_0).audio_clock - prev_audio_clock)
        ...
        if len1 < 0 as libc::c_int && ALIGN_AC3_PACKETS == 0 {
            ...
            *fresh14 = (*fresh14).offset(len1 as isize);
            ...
            if !(got_frame == 0) {
                ...
                (*is_0).audio_clock
                    += data_size_0 as libc::c_double
                        / ((*(*is_0).frame).channels * (*(*is_0).frame).sample_rate
    ...
    {
        ps = 0 as libc::c_int;
        rps = ((*pkt_temp).data).offset_from(ac3_packet.as_mut_ptr()) as libc::c_long
            as libc::c_int;
...
unsafe extern "C" fn print_fps(mut final_0: libc::c_int) -> libc::c_double {
    ...
    static mut tv_beg: timeval = timeval { tv_sec: 0, tv_usec: 0 };
    static mut tv_start: timeval = timeval { tv_sec: 0, tv_usec: 0 };
    static mut total_elapsed: libc::c_int = 0;
    static mut last_count: libc::c_int = 0 as libc::c_int;
    let mut tv_end: timeval = timeval { tv_sec: 0, tv_usec: 0 };
    ...
    elapsed = ((tv_end.tv_sec - tv_beg.tv_sec) * 100 as libc::c_int as libc::c_long
        ...
        as libc::c_int;
    total_elapsed = ((tv_end.tv_sec - tv_start.tv_sec)
        * 100 as libc::c_int as libc::c_long
        + ((tv_end.tv_usec - tv_start.tv_usec) / 10000 as libc::c_int) as libc::c_long)
        as libc::c_int;
    if final_0 != 0 {
        if total_elapsed != 0 {
            tfps = frame_counter as libc::c_double * 100.0f64
                / total_elapsed as libc::c_double;
        } else {
    ...
    fprintf(
        __stderrp,
        b"%s - %d frames in %.2f sec(%.2f fps), %.2f sec(%.2f fps), %d%%\r\0"
            as *const u8 as *const libc::c_char,
        cur_pos.as_mut_ptr(),
        frame_counter,
        total_elapsed as libc::c_double / 100.0f64,
        ...
        fps,
        (100.0f64 * framenum as libc::c_double / get_fps()
            / (*global_video_state).duration) as libc::c_int,
    );
    fflush(__stderrp);
...
pub unsafe extern "C" fn SubmitFrame(
    ...
    if (*pFrame).linesize[0 as libc::c_int as usize] > 4000 as libc::c_int
        || (*pFrame).height > 2400 as libc::c_int
        || (*pFrame).linesize[0 as libc::c_int as usize] < 100 as libc::c_int
        || (*pFrame).height < 100 as libc::c_int
    {
        frame_ptr = 0 as *mut libc::c_uchar;
        return 0 as libc::c_int;
    }
    if height != (*pFrame).height && (*pFrame).height > 100 as libc::c_int
        && (*pFrame).height < 2400 as libc::c_int
    {
        height = (*pFrame).height;
        changed = 1 as libc::c_int;
    }
    if width != (*pFrame).linesize[0 as libc::c_int as usize]
        && (*pFrame).linesize[0 as libc::c_int as usize] > 100 as libc::c_int
        && (*pFrame).linesize[0 as libc::c_int as usize] < 4000 as libc::c_int
    {
        width = (*pFrame).linesize[0 as libc::c_int as usize];
        changed = 1 as libc::c_int;
    }
    if videowidth != (*pFrame).width && (*pFrame).width > 100 as libc::c_int
        && (*pFrame).width < 4000 as libc::c_int
    {
    ...
    infopos = headerpos;
    frame_ptr = (*pFrame).data[0 as libc::c_int as usize];
    if frame_ptr.is_null() {
        return 0 as libc::c_int;
    }
    if (*pFrame).pict_type as libc::c_uint
        == AV_PICTURE_TYPE_B as libc::c_int as libc::c_uint
    {
    ...
    if selftest == 2 as libc::c_int && framenum == 0 as libc::c_int
        && pass == 0 as libc::c_int && test_pts == 0.0f64
    {
        test_pts = pts_0;
    }
...
pub unsafe extern "C" fn DoSeekRequest(mut is_0: *mut VideoState) {
    ...
    loop {
        ...
        if ((*(*(*is_0).pFormatCtx).iformat).read_seek).is_some() {
            error_text = b"Format specific\0" as *const u8 as *const libc::c_char
                as *mut libc::c_char;
        } else if ((*(*(*is_0).pFormatCtx).iformat).read_timestamp).is_some() {
            error_text = b"Frame binary\0" as *const u8 as *const libc::c_char
                as *mut libc::c_char;
        } else {
            error_text = b"Generic\0" as *const u8 as *const libc::c_char
                as *mut libc::c_char;
        }
        fprintf(
            __stderrp,
...
pub unsafe extern "C" fn DecodeOnePicture(mut f: *mut FILE, mut pts_0: libc::c_double) {
    ...
    's_49: while !((*is_0).quit != 0) {
        if (*is_0).seek_req != 0 {
            current_block_20 = 2959876918863150081;
        } else {
            current_block_20 = 14814866274374674359;
        }
        loop {
                ...
                _ => {
                    ...
                    let mut packet_time: libc::c_double = ((*packet).pts
                            ...
                            != 0x8000000000000000 as libc::c_ulonglong as int64_t
                        {
                            ...
                            0 as libc::c_int as libc::c_longlong
                        })) as libc::c_double * av_q2d((*(*is_0).video_st).time_base);
                    if (*packet).pts
                        == 0x8000000000000000 as libc::c_ulonglong as int64_t
                    {
                        ...
                        *fresh16 = (*fresh16 as libc::c_double
                            + ((*is_0).seek_pts - 2.5f64 - packet_time)
                                / (*is_0).duration
...
pub unsafe extern "C" fn filter() -> libc::c_int {
    printf(
        b"Exception raised, Comskip is terminating\n\0" as *const u8
            as *const libc::c_char,
    );
...
pub unsafe extern "C" fn video_packet_process(
    ...
    if frameFinished != 0 {
        if (*(*(*is_0).video_st).codec).framerate.den != 0
            && (*(*(*is_0).video_st).codec).framerate.num != 0
        {
            frame_delay = 1 as libc::c_int as libc::c_double
                / av_q2d((*(*(*is_0).video_st).codec).framerate);
        } else {
        ...
        calculated_delay = (best_effort_timestamp - pev_best_effort_timestamp)
            as libc::c_double * av_q2d((*(*is_0).video_st).time_base);
        ...
        if force_29fps != 0 && find_29fps == 5 as libc::c_int {
            frame_delay = 0.033366666666666669f64;
            Debug(
                1 as libc::c_int,
                b"Framerate forced %6.3f fps at frame %d\n\0" as *const u8
                    as *const libc::c_char as *mut libc::c_char,
                1.0f64 / frame_delay,
                frame_count,
            );
        ...
        if force_25fps != 0 && find_25fps == 5 as libc::c_int {
            frame_delay = 0.04f64;
            Debug(
                1 as libc::c_int,
                b"Framerate forced %6.3f fps at frame %d\n\0" as *const u8
        ...
        if force_24fps != 0 && find_24fps == 5 as libc::c_int {
                ...
                ) < 0.001f64)
                {
                    if !(fabs(
                        1 as libc::c_int as libc::c_double * frame_delay
                            / (*(*(*is_0).video_st).codec).ticks_per_frame
                                as libc::c_double - calculated_delay,
                    ) < 0.001f64)
                    {
        ...
        if reviewing == 0 && framenum > 1 as libc::c_int
            ...
            && !(fabs(
                2 as libc::c_int as libc::c_double * frame_delay
                    / (*(*(*is_0).video_st).codec).ticks_per_frame as libc::c_double
                    - calculated_delay,
            ) < 0.001f64)
            && !(fabs(
                ...
                && prev_strange_step < fabs(calculated_delay - frame_delay)
            {
                Debug(
                    ...
                    b"Strange video pts step of %6.5f instead of %6.5f at frame %d\n\0"
                        as *const u8 as *const libc::c_char as *mut libc::c_char,
                    calculated_delay + 0.0000005f64,
                    frame_delay + 0.0000005f64,
                    framenum,
                );
        ...
        if retries == 0 as libc::c_int {
            if (*is_0).video_clock - (*is_0).seek_pts > -frame_delay / 2.0f64 {
                ...
                if SubmitFrame((*is_0).video_st, (*is_0).pFrame, (*is_0).video_clock)
                    != 0
                {
                    current_block = 2409136188893683703;
                } else {
        ...
        } else if fabs((*is_0).seek_pts - (*is_0).video_clock)
            ...
            _ => {
                ...
                if (*(*is_0).pFrame).nb_side_data != 0 {
                    ...
                    while i < (*(*is_0).pFrame).nb_side_data {
                        let mut sd: *mut AVFrameSideData = *((*(*is_0).pFrame).side_data)
                            .offset(i as isize);
                            ...
                            != AV_FRAME_DATA_A53_CC as libc::c_int as libc::c_uint)
                        {
                            ...
                            ccData[4 as libc::c_int
                                as usize] = 3 as libc::c_int as uint8_t;
                            ccData[5 as libc::c_int
                                as usize] = ((*sd).size / 3 as libc::c_int
                                + 64 as libc::c_int) as uint8_t;
                            i = 0 as libc::c_int;
                            while i < (*sd).size {
                                ccData[(i + 7 as libc::c_int)
                                    as usize] = *((*sd).data).offset(i as isize);
                                i += 1;
                            }
                            dump_data(
                                ccData.as_mut_ptr() as *mut libc::c_char,
                                ccDataLen,
                            );
                            if processCC != 0 {
                                ProcessCCData();
...
pub unsafe extern "C" fn stream_component_open(
    ...
    let mut codec_hw: *mut AVCodec = 0 as *mut AVCodec;
    ...
    if (*codecCtx).codec_type as libc::c_int == AVMEDIA_TYPE_VIDEO as libc::c_int {
        ...
        let ref mut fresh18 = (*is_0).dec_ctx;
        *fresh18 = codecCtx;
    ...
    if hardware_decode != 0 {
        if (*codecCtx).codec_id as libc::c_uint
            ...
            && !(avcodec_find_decoder_by_name(
                b"mpeg2_mmal\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_H264 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"h264_mmal\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_MPEG4 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"mpeg4_mmal\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_VC1 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"vc1_mmal\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
    ...
    if use_cuvid != 0 {
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_MPEG2VIDEO as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"mpeg2_cuvid\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_H264 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"h264_cuvid\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_HEVC as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"hevc_cuvid\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_MPEG4 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"mpeg4_cuvid\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_VC1 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"vc1_cuvidl\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
    ...
    if use_vdpau != 0 {
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_MPEG2VIDEO as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"mpeg2_vdpau\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_H264 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"h264_vdpau\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_MPEG4 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"mpeg4_vdpau\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_VC1 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"vc1_vdpau\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
    ...
    if use_dxva2 != 0 {
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_MPEG2VIDEO as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"mpeg2_dxva2\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_H264 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"h264_dxva2\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_MPEG4 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"mpeg4_dxva2\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
        ...
        if (*codecCtx).codec_id as libc::c_uint
            == AV_CODEC_ID_VC1 as libc::c_int as libc::c_uint
            && !(avcodec_find_decoder_by_name(
                b"vc1_dxva2\0" as *const u8 as *const libc::c_char,
            ))
                .is_null()
        {
            codec_hw = avcodec_find_decoder_by_name(
    ...
    if !codec_hw.is_null() && codec_hw != codec {
        fprintf(
            __stderrp,
            b"Using Codec: %s instead of %s\n\0" as *const u8 as *const libc::c_char,
            (*codec_hw).name,
            (*codec).name,
        );
        codec = codec_hw;
    ...
    if codec.is_null()
        || avcodec_open2(codecCtx, codec, &mut myoptions) < 0 as libc::c_int
    {
        fprintf(
            __stderrp,
            b"Unsupported codec!\n\0" as *const u8 as *const libc::c_char,
    ...
    match (*codecCtx).codec_type as libc::c_int {
        3 => {
            ...
            let ref mut fresh19 = (*is_0).subtitle_st;
            *fresh19 = *((*pFormatCtx).streams).offset(stream_index_0 as isize);
            if demux_pid != 0 {
        ...
        1 => {
            ...
            let ref mut fresh20 = (*is_0).audio_st;
            *fresh20 = *((*pFormatCtx).streams).offset(stream_index_0 as isize);
            if demux_pid != 0 {
        ...
        0 => {
            ...
            let ref mut fresh21 = (*is_0).video_st;
            *fresh21 = *((*pFormatCtx).streams).offset(stream_index_0 as isize);
            let ref mut fresh22 = (*is_0).pFrame;
            *fresh22 = av_frame_alloc();
            if hardware_decode == 0 {
                (*codecCtx).flags |= (1 as libc::c_int) << 13 as libc::c_int;
            }
...
pub unsafe extern "C" fn file_open() {
    ...
    if global_video_state.is_null() {
        is_0 = av_mallocz(::std::mem::size_of::<VideoState>() as libc::c_ulong)
            as *mut VideoState;
        memset(
        ...
        let ref mut fresh23 = (*is_0).pFormatCtx;
        *fresh23 = 0 as *mut AVFormatContext;
        if hardware_decode == 0 {
            av_dict_set_int(
                &mut myoptions,
                b"gray\0" as *const u8 as *const libc::c_char,
    ...
    if ((*is_0).pFormatCtx).is_null() {
        let ref mut fresh24 = (*is_0).pFormatCtx;
        *fresh24 = avformat_alloc_context();
        let ref mut fresh25 = (*(*is_0).pFormatCtx).max_analyze_duration;
        *fresh25 *= 4 as libc::c_int as libc::c_longlong;
        's_142: {
            loop {
                ...
                ) != 0 as libc::c_int
                {
                    ...
                    let fresh26 = openretries;
                    openretries = openretries + 1;
                    if fresh26 < live_tv_retries {
                        usleep(
                            (1000 as libc::c_long * 1000 as libc::c_long) as useconds_t,
                        );
                    } else {
                        exit(-(1 as libc::c_int));
                    }
        ...
        (*is_0)
            .seek_by_bytes = ((*(*(*is_0).pFormatCtx).iformat).flags
    ...
    if (*is_0).videoStream == -(1 as libc::c_int) {
        ...
        if (*(*is_0).video_st).duration
            == 0x8000000000000000 as libc::c_ulonglong as int64_t
            || (*(*is_0).video_st).duration < 0 as libc::c_int as libc::c_longlong
        {
            (*is_0)
                .duration = ((*(*is_0).pFormatCtx).duration as libc::c_float
                / 1000000 as libc::c_int as libc::c_float) as libc::c_double;
        } else {
            (*is_0)
                .duration = av_q2d((*(*is_0).video_st).time_base)
        ...
        if (*(*is_0).video_st).r_frame_rate.den != 0
            && (*(*is_0).video_st).r_frame_rate.num != 0
        {
            (*is_0).fps = av_q2d((*(*is_0).video_st).r_frame_rate);
        } else {
            Debug(
                10 as libc::c_int,
                b"Warning, no stream frame rate, deriving from codec\n\0" as *const u8
                    as *const libc::c_char as *mut libc::c_char,
            );
            (*is_0)
                .fps = 1 as libc::c_int as libc::c_double
                / (av_q2d((*(*(*is_0).video_st).codec).time_base)
                    * (*(*(*is_0).video_st).codec).ticks_per_frame as libc::c_double);
        }
        set_fps(1.0f64 / (*is_0).fps);
    }
    ...
    if (*is_0).subtitleStream == -(1 as libc::c_int) && video_index >= 0 as libc::c_int {
        ...
        if subtitle_index >= 0 as libc::c_int {
            let ref mut fresh27 = (*is_0).subtitle_st;
            *fresh27 = *((*(*is_0).pFormatCtx).streams).offset(subtitle_index as isize);
            if demux_pid != 0 {
                selected_subtitle_pid = (*(*is_0).subtitle_st).id;
            }
        }
...
pub unsafe extern "C" fn file_close() {
    ...
    if (*is).videoStream != -(1 as libc::c_int) {
        avcodec_close(
            (**((*(*is).pFormatCtx).streams).offset((*is).videoStream as isize)).codec,
        );
    }
    (*is).videoStream = -(1 as libc::c_int);
    if (*is).audioStream != -(1 as libc::c_int) {
        avcodec_close(
            (**((*(*is).pFormatCtx).streams).offset((*is).audioStream as isize)).codec,
        );
    }
...
unsafe fn main_0(
    ...
    if ptr.is_null()
        || ptr.offset_from(HomeDir.as_mut_ptr()) as libc::c_long
            == 0 as libc::c_int as libc::c_long
    {
    ...
    fprintf(__stderrp, b"Donator build\n\0" as *const u8 as *const libc::c_char);
    ...
    while !((*is).quit != 0) {
        ...
        loop {
            ...
            packet_time = ((*packet).pts
                    ...
                    != 0x8000000000000000 as libc::c_ulonglong as int64_t
                {
                    ...
                    0 as libc::c_int as libc::c_longlong
                })) as libc::c_double * av_q2d((*(*is).video_st).time_base);
                ...
                || (*packet).pts == 0 as libc::c_int as libc::c_longlong
            {
                av_packet_unref(packet);
                    ...
                    && fabs(packet_time - ((*is).seek_pts - 2.5f64))
                        < (*is).duration
                            / (10 as libc::c_int * (*is).seek_req) as libc::c_double
                {
        ...
        match current_block {
            ...
            12758904613967585247 => {
                let ref mut fresh28 = (*is).seek_pos;
                *fresh28 = (*fresh28 as libc::c_double
                    + ((*is).seek_pts - 2.5f64 - packet_time) / (*is).duration
                        * avio_size((*(*is).pFormatCtx).pb) as libc::c_double * 0.9f64)
                    as int64_t;
                let ref mut fresh29 = (*is).seek_req;
                *fresh29 += 1;
                continue;
            }
            _ => {}
        ...
        (*is).seek_req = 0 as libc::c_int;
        if selftest == 3 as libc::c_int && retries == 0 as libc::c_int
            && (*is).video_clock >= 500.0f64
        {
            ret = -((('E' as i32 | ('O' as i32) << 8 as libc::c_int
        ...
        if selftest == 4 as libc::c_int && retries == 0 as libc::c_int
            && framenum > 0 as libc::c_int
        ...
        if ret < 0 as libc::c_int {
                ...
                || (*(*(*is).pFormatCtx).pb).eof_reached != 0
            {
                ...
                let mut frame_delay: libc::c_double = av_q2d(
        ...
        if selftest == 1 as libc::c_int && pass == 0 as libc::c_int
            && (*is).seek_req == 0 as libc::c_int && framenum == 50 as libc::c_int
        {
