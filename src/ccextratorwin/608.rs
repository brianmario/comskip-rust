...
#[no_mangle]
pub static mut subline: *mut libc::c_uchar = 0 as *const libc::c_uchar
    as *mut libc::c_uchar;
#[no_mangle]
pub static mut new_sentence: libc::c_int = 1 as libc::c_int;
#[no_mangle]
pub static mut usercolor_rgb: [libc::c_uchar; 8] = unsafe {
...
#[no_mangle]
pub unsafe extern "C" fn correct_case(
    mut line_num: libc::c_int,
    mut data: *mut eia608_screen,
) {
    let mut i: libc::c_int = 0 as libc::c_int;
    while i < spell_words {
        let mut c: *mut libc::c_char = ((*data).characters[line_num as usize])
        ...
        loop {
            ...
            let mut prev: libc::c_uchar = (if c
                == ((*data).characters[line_num as usize]).as_mut_ptr()
                    as *mut libc::c_char
            {
                ' ' as i32
            } else {
...
#[no_mangle]
pub unsafe extern "C" fn capitalize(
    mut line_num: libc::c_int,
    mut data: *mut eia608_screen,
) {
...
pub unsafe extern "C" fn get_decoder_line_encoded(
    ...
    let mut line: *mut libc::c_uchar = ((*data).characters[line_num as usize])
        .as_mut_ptr();
    let mut orig: *mut libc::c_uchar = buffer;
    i = 0 as libc::c_int;
    while i < 32 as libc::c_int {
        let mut its_col: libc::c_int = (*data).colors[line_num as usize][i as usize]
            as libc::c_int;
        if its_col != col && nofontcolor == 0 {
            ...
            buffer = buffer
                .offset(
                    encode_line(
                        buffer,
                        color_text[its_col as usize][1 as libc::c_int as usize]
                            as *mut libc::c_uchar,
                    ) as isize,
                );
            if its_col == COL_USERDEFINED as libc::c_int {
                buffer = buffer
                    .offset(encode_line(buffer, usercolor_rgb.as_mut_ptr()) as isize);
                buffer = buffer
                    .offset(
                        encode_line(
                            buffer,
...
pub unsafe extern "C" fn clear_eia608_cc_buffer(mut data: *mut eia608_screen) {
    ...
    while i < 15 as libc::c_int {
        ...
        (*data)
            .characters[i
            as usize][32 as libc::c_int as usize] = 0 as libc::c_int as libc::c_uchar;
        memset(
...
pub unsafe extern "C" fn write_char(c: libc::c_uchar, mut wb: *mut s_write) {
    if (*(*wb).data608).mode != MODE_TEXT as libc::c_int {
        ...
        (*use_buffer)
            .characters[(*(*wb).data608).cursor_row
            as usize][(*(*wb).data608).cursor_column as usize] = c;
        (*use_buffer)
            .colors[(*(*wb).data608).cursor_row
            as usize][(*(*wb).data608).cursor_column as usize] = (*(*wb).data608).color;
        (*use_buffer)
            .fonts[(*(*wb).data608).cursor_row
            as usize][(*(*wb).data608).cursor_column as usize] = (*(*wb).data608).font;
...
pub unsafe extern "C" fn handle_text_attr(
    ...
    if (c1 as libc::c_int != 0x11 as libc::c_int
        && c1 as libc::c_int != 0x19 as libc::c_int
        || ((c2 as libc::c_int) < 0x20 as libc::c_int
            || c2 as libc::c_int > 0x2f as libc::c_int)) && debug_608 != 0
    {
        printf(
            b"\rThis is not a text attribute!\n\0" as *const u8 as *const libc::c_char,
        );
    } else {
...
pub unsafe extern "C" fn mstotime(
    ...
    *seconds = (milli % 60 as libc::c_int as libc::c_long) as libc::c_int
        as libc::c_uint;
    milli = (milli - *seconds as libc::c_long) / 60 as libc::c_int as libc::c_long;
    *minutes = (milli % 60 as libc::c_int as libc::c_long) as libc::c_int
        as libc::c_uint;
...
#[no_mangle]
pub unsafe extern "C" fn fprintf_encoded(
    mut fh: *mut FILE,
...
pub unsafe extern "C" fn write_cc_buffer_as_srt(
    ...
    let mut ms_start: LONG = (((*(*wb).data608).current_visible_start_cc)
        .wrapping_add(pg)
        .wrapping_mul(1000 as libc::c_int as libc::c_uint) as libc::c_double / 29.97f64)
        as LONG;
    ...
    *fresh2 += 1;
    ...
    if debug_608 != 0 {
        ...
        printf(b"%s\0" as *const u8 as *const libc::c_char, timeline.as_mut_ptr());
    }
    fwrite(
        enc_buffer.as_mut_ptr() as *const libc::c_void,
        enc_buffer_used as libc::c_ulong,
...
pub unsafe extern "C" fn write_cc_buffer_as_sami(
    ...
    while i < 15 as libc::c_int {
        if (*data).row_used[i as usize] != 0 {
            let mut length: libc::c_int = get_decoder_line_encoded(subline, i, data)
                as libc::c_int;
            if debug_608 != 0 && encoding != ENC_UNICODE as libc::c_int {
                printf(b"\r\0" as *const u8 as *const libc::c_char);
                printf(b"%s\n\0" as *const u8 as *const libc::c_char, subline);
    ...
    if i > 0 as libc::c_int {
        ...
        while j < lastrow {
            if j >= 0 as libc::c_int {
                memcpy(
                    ((*use_buffer).characters[j as usize]).as_mut_ptr()
                        as *mut libc::c_void,
                    ((*use_buffer).characters[(j + 1 as libc::c_int) as usize])
                ...
                (*use_buffer)
                    .row_used[j
                    as usize] = (*use_buffer).row_used[(j + 1 as libc::c_int) as usize];
            }
            j += 1;
        }
        j = 0 as libc::c_int;
...
pub unsafe extern "C" fn handle_command(
    ...
    if (c1 as libc::c_int == 0x14 as libc::c_int
        || c1 as libc::c_int == 0x1c as libc::c_int)
    ...
    if (c1 as libc::c_int == 0x17 as libc::c_int
        || c1 as libc::c_int == 0x1f as libc::c_int)
    ...
    match command {
            ...
            if (*(*wb).data608).cursor_column > 0 as libc::c_int {
                ...
                *fresh3 -= 1;
                (*get_writing_buffer(wb))
                    .characters[(*(*wb).data608).cursor_row
                    as usize][(*(*wb).data608).cursor_column
        ...
        3 => {
            ...
            (*(*wb).data608)
                .visible_buffer = if (*(*wb).data608).visible_buffer == 1 as libc::c_int
            {
                2 as libc::c_int
            } else {
                1 as libc::c_int
...
#[no_mangle]
pub unsafe extern "C" fn handle_double(
    c1: libc::c_uchar,
    c2: libc::c_uchar,
    mut wb: *mut s_write,
) {
    ...
    if c2 as libc::c_int >= 0x30 as libc::c_int
        && c2 as libc::c_int <= 0x3f as libc::c_int
    {
...
pub unsafe extern "C" fn handle_extended(
    ...
    if lo as libc::c_int >= 0x20 as libc::c_int
        ...
        && (hi as libc::c_int == 0x12 as libc::c_int
            || hi as libc::c_int == 0x13 as libc::c_int)
    {
...
pub unsafe extern "C" fn handle_pac(
    ...
    let mut row: libc::c_int = rowdata[((c1 as libc::c_int) << 1 as libc::c_int
        & 14 as libc::c_int | c2 as libc::c_int >> 5 as libc::c_int & 1 as libc::c_int)
        as usize];
    if debug_608 != 0 {
        printf(
            b"\rPAC: %02X %02X\n\0" as *const u8 as *const libc::c_char,
    ...
    if c2 as libc::c_int >= 0x40 as libc::c_int
        && c2 as libc::c_int <= 0x5f as libc::c_int
    {
        c2 = (c2 as libc::c_int - 0x40 as libc::c_int) as libc::c_uchar;
    } else if c2 as libc::c_int >= 0x60 as libc::c_int
            && c2 as libc::c_int <= 0x7f as libc::c_int
        {
...
pub unsafe extern "C" fn handle_single(c1: libc::c_uchar, mut wb: *mut s_write) {
    if (c1 as libc::c_int) < 0x20 as libc::c_int
        || (*(*wb).data608).channel != cc_channel
    {
        return;
    }
    write_char(c1, wb);
...
pub unsafe extern "C" fn check_channel(mut c1: libc::c_uchar, mut wb: *mut s_write) {
    if (*(*wb).data608).channel != 1 as libc::c_int
        && c1 as libc::c_int == 0x14 as libc::c_int
    {
        if debug_608 != 0 {
    ...
    if (*(*wb).data608).channel != 2 as libc::c_int
        && c1 as libc::c_int == 0x1c as libc::c_int
    {
        if debug_608 != 0 {
    ...
    if (*(*wb).data608).channel != 3 as libc::c_int
        && (c1 as libc::c_int >= 0x1 as libc::c_int
            && c1 as libc::c_int <= 0xf as libc::c_int
    ...
    if (*(*wb).data608).channel != 4 as libc::c_int
        && c1 as libc::c_int == 0x1d as libc::c_int
    {
        if debug_608 != 0 {
...
pub unsafe extern "C" fn disCommand(
    ...
    if hi as libc::c_int >= 0x18 as libc::c_int
    ...
    match hi as libc::c_int {
        16 => {
            if lo as libc::c_int >= 0x40 as libc::c_int
                && lo as libc::c_int <= 0x5f as libc::c_int
            {
                handle_pac(hi, lo, wb);
            }
        }
        17 => {
            if lo as libc::c_int >= 0x20 as libc::c_int
                && lo as libc::c_int <= 0x2f as libc::c_int
            {
                handle_text_attr(hi, lo, wb);
            }
            if lo as libc::c_int >= 0x30 as libc::c_int
                && lo as libc::c_int <= 0x3f as libc::c_int
            {
            ...
            if lo as libc::c_int >= 0x40 as libc::c_int
                && lo as libc::c_int <= 0x7f as libc::c_int
            {
                handle_pac(hi, lo, wb);
            }
        }
        ...
        23 => {
            if lo as libc::c_int >= 0x21 as libc::c_int
                && lo as libc::c_int <= 0x22 as libc::c_int
            {
                handle_command(hi, lo, wb);
            }
            if lo as libc::c_int >= 0x2e as libc::c_int
                && lo as libc::c_int <= 0x2f as libc::c_int
            {
                handle_text_attr(hi, lo, wb);
            }
...
pub unsafe extern "C" fn process608(
    ...
    if !data.is_null() {
        ...
        while i < length {
            ...
            let mut wrote_to_screen: libc::c_int = 0 as libc::c_int;
            hi = (*data.offset(i as isize) as libc::c_int & 0x7f as libc::c_int)
                as libc::c_uchar;
            lo = (*data.offset((i + 1 as libc::c_int) as isize) as libc::c_int
                & 0x7f as libc::c_int) as libc::c_uchar;
            if !(hi as libc::c_int == 0 as libc::c_int
                && lo as libc::c_int == 0 as libc::c_int)
            {
                check_channel(hi, wb);
                if !((*(*wb).data608).channel != cc_channel) {
                    ...
                    if hi as libc::c_int == 0xf as libc::c_int {
                        ...
                        {
                            in_xds_mode = 0 as libc::c_int;
                            if (*(*wb).data608).last_c1 as libc::c_int
                                == hi as libc::c_int
                                && (*(*wb).data608).last_c2 as libc::c_int
                                    == lo as libc::c_int
                            {
                        ...
                        match current_block_22 {
                            ...
                            _ => {
                                if hi as libc::c_int >= 0x20 as libc::c_int {
                                    ...
                                    wrote_to_screen = 1 as libc::c_int;
                                    (*(*wb).data608)
                                        .last_c1 = 0 as libc::c_int as libc::c_uchar;
                                    (*(*wb).data608)
                                        .last_c2 = 0 as libc::c_int as libc::c_uchar;
                                }
                                if wrote_to_screen != 0 && direct_rollup != 0
                                    && ((*(*wb).data608).mode == MODE_ROLLUP_2 as libc::c_int
                                        || (*(*wb).data608).mode == MODE_ROLLUP_3 as libc::c_int
                                        || (*(*wb).data608).mode == MODE_ROLLUP_4 as libc::c_int)
                                {
                                    write_cc_buffer(wb);
                                    (*(*wb).data608)
                                        .current_visible_start_cc = totalblockswritten_thisfile();
                                }
                            }
                        }
                    }
